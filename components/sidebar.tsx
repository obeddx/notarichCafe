...
const Sidebar: React.FC<SidebarProps> = ({ onToggle, isOpen }) => {
  ...
  return (
    <div
      className={`fixed top-0 left-0 h-full bg-[#212121] shadow-lg z-50 transition-all duration-300 overflow-y-auto ${
        isOpen ? "w-64" : "w-20"
      }`}
    >
      <div className="flex flex-col items-center py-6 relative">
        ...
        <div
          className={`my-4 h-32 overflow-hidden rounded-full transition-all ${
            isOpen ? "w-32" : "w-16"
          }`}
        >
          <Image
      ...
      <ul className="mt-6 space-y-2">
        ...
        <li>
          <SidebarDropdown
            label="Menu Notarich"
            isOpen={isOpen}
            items={[{ href: "/manager/getMenu", label: "Daftar Menu" }]}
            border
          />
        </li>
        <li>
...
const SidebarItem: React.FC<SidebarItemProps> = ({
  ...
  let classes = `text-white text-md py-2 px-4 rounded-md transition-all cursor-pointer hover:bg-[#FF8A00] ${
  ...
  return (
    <div className={`mx-4 ${border ? "border-y-2 border-neutral-700" : ""}`}>
      <div
        ...
        className={`flex justify-between items-center text-white text-md py-2 px-4 rounded-md transition-all cursor-pointer hover:bg-[#FF8A00] ${
          isActive ? "bg-[#FF8A00] font-bold" : ""
        }`}
      >
