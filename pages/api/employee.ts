...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === "POST") {
    try {
      ...
      // Generate token tanpa masa berlaku
      const inviteToken = crypto.randomBytes(16).toString("hex");
      ...
      const newEmployee = await prisma.employee.create({
        data: {
          ...
          expiredDate: new Date(expiredDate),
          inviteToken,
          // inviteExpiresAt dihapus atau biarkan null jika masih ada di skema
        },
        include: { role: true },
      });
      ...
      if (email) {
        ...
        await transporter.sendMail({
          ...
          subject: "Selamat Datang di Notarich Cafe",
          text: `Halo ${firstName} ${lastName}, selamat datang di keluarga Notarich Cafe dengan job sebagai ${newEmployee.role?.name}.
Silakan registrasi akunmu melalui link berikut: ${registerLink}.
Terima Kasih!`,
        });
      }
