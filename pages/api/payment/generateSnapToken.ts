...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const { orderId, total, customerName, customerEmail, customerPhone, item_details } = req.body;
    ...
    // Siapkan parameter transaksi termasuk item_details
    const parameter = {
      transaction_details: {
        order_id: orderId,
        gross_amount: total,
      },
      // Item details: array of objects (pastikan data ini dikirim dari frontend)
      item_details: item_details || [],
      customer_details: {
        first_name: customerName,
        email: customerEmail && customerEmail.includes("@") ? customerEmail : "customer@example.com",
        phone: customerPhone || "",
      },
      expiry: {
