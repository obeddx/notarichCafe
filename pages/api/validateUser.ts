...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // Untuk role lainnya, pertama coba di tabel User
    ...
    // Fallback: jika tidak ditemukan di tabel User, coba cek di tabel Employee (invite token)
    const employee = await prisma.employee.findFirst({
      where: { inviteToken: userToken },
      include: { role: true },
    });
    ...
    if (!employee.role || employee.role.name.toLowerCase() !== role.toLowerCase()) {
      return res.status(403).json({ message: "Forbidden: Role mismatch" });
    }

    return res.status(200).json({ message: "Authorized", user: employee });
  } catch (error) {
    console.error("Validation error:", error);
    return res.status(500).json({ message: "Internal server error" });
  } finally {
