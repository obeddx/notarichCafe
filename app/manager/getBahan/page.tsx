...
export default function IngredientsTable() {
  ...
  const rawIngredientOptions = rawIngredientsList.map((raw) => ({
    value: raw.id,
    label: `${raw.name} (${raw.unit})`,
  }));
  ...
  const handleSemiEditCompositionChange = (
    index: number,
    field: keyof SemiComposition,
    value: string | number
  ) => {
    const newComposition = [...semiEditComposition];
    newComposition[index] = {
      ...newComposition[index],
      [field]:
        field === "rawIngredientId"
    ...
    setSemiEditComposition(newComposition);
  };
  ...
  {semiEditComposition.map((row, index) => {
    ...
    return (
      <div key={index} className="flex gap-2 mb-2 items-center">
       <Select
                        options={rawIngredientOptions}
                        value={rawIngredientOptions.find(option => option.value === row.rawIngredientId) || null}
                        onChange={(selectedOption) =>
                          handleSemiEditCompositionChange(index, "rawIngredientId", selectedOption ? selectedOption.value : "")
                        }
                        placeholder="Pilih Raw Ingredient"
                        isSearchable={true}
                        required
                        className="flex-5"
                  ...
                  return (
                    <div key={index} className="flex gap-2 mb-2 items-center">
                      <Select
                        ...
                        value={rawIngredientOptions.find(option => option.value === row.rawIngredientId) || null}
                        onChange={(selectedOption) =>
                          handleCompositionChange(index, "rawIngredientId", selectedOption ? selectedOption.value : "")
                        }
                        placeholder="Pilih Raw Ingredient"
                        isSearchable={true}
                        required
