...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (res.ok) {
        // Cek apakah peran user sesuai dengan role yang terkunci di query parameter
        if (data.user.role.toLowerCase() !== lockedRole.toLowerCase()) {
          setErrorMessage(`Akun Anda tidak memiliki akses ${lockedRole}`);
          setLoading(false);
          return;
        }
        ...
        if (lockedRole === "manager") {
          router.push("/manager");
        } else if (lockedRole === "kasir") {
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-cover bg-center p-4" style={{ backgroundImage: "url('/login2.png')" }}>
      <div className="relative w-full max-w-md p-6 bg-white bg-opacity-80 rounded-lg shadow-lg md:max-w-lg lg:max-w-xl">
        ...
        <h2 className="text-2xl font-bold text-center mb-4 text-black">Log In {lockedRole?.toLowerCase()}</h2>
